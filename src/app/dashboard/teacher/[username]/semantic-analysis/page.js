...
// small helper to turn display name/email into URL-safe username
function slugifyUsername(user) {
  const base =
...
export default function UploadPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = res.data;
      localStorage.setItem(`semantic:${data.id}`, JSON.stringify(data));

      const username = slugifyUsername(session?.user || {});
      router.push(
        `/dashboard/teacher/${username}/semantic-analysis/analysis-report/${data.id}`
      );
    } catch (err) {
      console.error("Semantic upload error:", err?.response || err);
      setError(
  ...
  return (
    <>
      <title>Upload Documents - Semantic Analysis</title>
      ...
      <div className="h-auto bg-gradient-to-r from-black to-gray-900 text-gray-300 py-44">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <p
            className={`${rw.className} px-12 text-lg md:text-xl text-center mb-12 text-gray-400`}
          >
            Go beyond word-for-word matching. This analysis detects paraphrased
            or rephrased content by understanding the meaning and context of
            sentences.
          </p>
