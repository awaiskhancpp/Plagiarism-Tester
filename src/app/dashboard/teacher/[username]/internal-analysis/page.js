...
export default function UploadPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = res.data;
      localStorage.setItem(`internal:${data.id}`, JSON.stringify(data));
      router.push(
        `/dashboard/teacher/${session.user.name.replace(
          " ",
          "-"
        )}/internal-analysis/analysis-report/${data.id}`
      );
    } catch (err) {
      console.error(err);
    ...
    <>
      ...
      <div className="h-auto bg-gradient-to-r from-black to-gray-900 text-gray-300 py-44">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <p
            className={`${rw.className} text-lg md:text-xl px-14 text-center mb-12 text-gray-400`}
          >
            Upload multiple student documents to run an internal plagiarism
            check. Our system compares each file against the others in the
            batch, helping teachers quickly detect copied or overlapping content
